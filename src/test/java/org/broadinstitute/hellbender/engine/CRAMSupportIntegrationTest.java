...
public final class CRAMSupportIntegrationTest extends CommandLineProgramTest{
    ...
    public void testSamtoolsGeneratedCRAMSliceMD5Calculation() throws IOException {
        // The input cram was generated using samtools and the "ambiguityCodes.fasta" file as the reference. But
        // that reference contains ambiguity codes (which is essential, since part of what we're trying to validate
        // is that htsjdk calculates the same MD5 value for a slice who's reference spans ambiguity codes as samtools does).
        // But, GATK wants a sequence dictionary to read this file, which is also included here, but the .dict was generated
        ...
        runCommandLine(args);  // no assert, just make usre we don't throw
    }
